...
import cl.previred.rest.response.Status;
...
public class AppUtils {
	...
	public static Status getExceptionStatus(Integer codigo, Exception e) {
		Status status = new Status();
		status.setCode(codigo);
		status.setMessage(e.getMessage());
		status.setException(e.getClass().getName());
		return status;
	}

	public static Status getStatusOK() {
		Status status = new Status();
		status.setCode(AppConstants.SUCCESS);
		status.setMessage(AppConstants.OK_RESPONSE);
		status.setException("N/A");
		return status;
	}
